...
namespace MineDetonator
{
    public class Core : BaseSettingsPlugin<Settings>
    {
        ...
        public override void Render()
        {
            ...
            {
                bool CanDetonate(DeployedObject deployed)
                {
                    var entity = deployed?.Entity;
                    if (entity == null || !entity.IsValid) return false;
                    var stats = entity.GetComponent<Stats>();
                    return stats is not { StatDictionary: { Count: > 0 } } ||
                           !stats.StatDictionary.TryGetValue(GameStat.CannotDie, out var value) || value == 0;
                }

                if (deployedObjects.Any(CanDetonate))
                {
