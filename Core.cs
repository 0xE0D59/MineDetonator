        ...
        public override void Render()
        {
            ...
            bool IsValidMonster(Entity entity)
            {
                ...
                if (entity.HasComponent<Buffs>())
                {
                    ...
                    if (buffs == null)
                        throw new Exception("HasComponent<Buffs>() == true, but GetComponent<Buffs>() == null");
                    var buffsList = buffs.BuffsList;
                    if (buffsList.Count > 0 && IgnoreBuffs.Any(b1 =>
                        buffsList.Any(b2 => string.Equals(b1, b2.Name, StringComparison.OrdinalIgnoreCase)))) return false;
                }

                if (!entity.HasComponent<Actor>()) return false;
                var actor = entity.GetComponent<Actor>();
                if (!FilterNullAction(actor)) return false;
                if (actor.CurrentAction?.Skill?.Name == "AtziriSUmmonDemons") return false;
                if (actor.CurrentAction?.Skill?.Id == 728) return false;
